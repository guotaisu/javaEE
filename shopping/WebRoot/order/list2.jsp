	...
	<div class="main">
		...
		<tr>
			...
			<tr>
				...
				<td>
					<s:if test="(#listo.status==1)">已付款 </s:if>
					<s:if test="(#listo.status==2)">未付款 </s:if>
					<s:if test="(#listo.status==3)">已发货 </s:if>
				</td>
				<td><s:if test="(#listo.status==2)"><a href="payorder.action?oid=<s:property value="id"/>">付款</a></s:if></td>
			</tr>
			</s:iterator>
		</table>
