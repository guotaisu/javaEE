...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
		<div id="wrap">
			...
			<img src="images/guanggao_1.gif" />
			<!-- TemplateEndEditable -->
			<ul id="brands">
				<s:iterator value="topTypeList">
					<li>
						<a
							href="showGoodsByType.action?goodsType=<s:property value="id"/>">
			...
			<div class="container">
				<div class="global_l">
								...
								<li>
									<p class="img">
										<a href="{$goods.url}"><img src="{$goods.thumb}"
												alt="{$goods.name|escape:html}" class="goodsimg" /> </a>
									</p>
									<p class="name">
							...
							<ul>
								...
								<li>
									...
									<a href="{$item.url}"
										...
										style="background: none; padding-left: 0px;">{$item.act_name}</a>
								</li>
								<!-- {/foreach} -->
							</ul>
						</div>
					...
					<div class="module">
						<h1>
							<span></span>{$lang.email_subscribe}
						</h1>
						<div class="content" style="padding: 5px;">
							<input type="text" id="user_email" class="input_1"
								style="margin-bottom: 5px;" />
							<input type="button" value="{$lang.email_list_ok}"
								onclick="add_email_list();" class="button_1" />
							<input type="button" value="{$lang.email_list_cancel}"
								onclick="cancel_email_list();" class="button_2" />
						</div>
					</div>
  ...
  function check_email()
  {
    ...
    else
    {
      ...
      return false;
					...
					<div class="module">
						<h1>
							<span></span>{$lang.order_query}
						</h1>
						<div class="content" style="padding: 5px;">
							<form name="ecsOrderQuery">
								<input type="text" name="order_sn" class="input_1"
									style="margin-bottom: 5px;" />
								<input type="button" value="{$lang.query_order}"
							...
							<div id="ECS_ORDER_QUERY" style="margin-top: 8px;">
								...
								<b>{$lang.order_number}：</b><a
									...
									class="f6">{$order_query.order_sn}</a>
								<br>
									<!--{else}-->
										...
										<b>{$lang.order_status}：</b><br>
											<font class="f1">{$order_query.order_status}</font><br>
												...
												<b>{$lang.consignment}：</b>{$order_query.invoice_no}<br>
													<!--{/if}-->
													{if $order_query.shipping_date}：{$lang.shipping_date}
													{$order_query.shipping_date}<br>
														<!--{/if}-->
														<!--{/if}-->
							</div>
						</div>
					...
					<div class="module">
						<h1>
							<span></span>{$lang.shipping_query}
						</h1>
						<div class="content" style="padding: 5px;">
							<!-- 发货单查询{foreach from=$invoice_list item=invoice} -->
							{$lang.order_number} {$invoice.order_sn}
							<br />
						...
						<div class="content">
							...
							<ul class="goods_grid">
								<li>
									<p class="img">
										<a href="{$goods.url}"><img src="{$goods.thumb}"
												alt="{$goods.goods_name|escape:html}" class="goodsimg" /> </a>
									</p>
									<p class="name">
					...
					<div class="area">
						<div style="width: 198px; float: right;">
							...
							<div class="module" id="new_articles">
								<ul id="newArticleTab">
									<span></span>
									<li id="ECS_NOTICE" class="tabFront">
										{$lang.shop_notice}
									</li>
									<li id="ECS_ARTICLE" class="tabBack">
										{$lang.new_article}
									</li>
								</ul>
								<div class="content">
									...
									<div id="ECS_ARTICLE_BODY" style="display: none;">
										<ul>
											...
											<li>
												[
												<a href="{$article.cat_url}">{$article.cat_name}</a>]
												<a href="{$article.url}"
													title="{$article.title|escape:html}">{$article.short_title|truncate:9}</a>
											</li>
											<!-- {/if} -->
											<!-- {/foreach} -->
										</ul>
              ...
              var swf_height=218;
							...
							<script type="text/javascript"
								src="data/flashdata/{$flash_theme}/cycle_image.js"></script>
						</div>
					</div>

					<div class="module">
						<div class="caption">
							<span><a href=""> 更多>> </a> </span>电子商品
						</div>
						<div class="content">
							<ul class="goods_grid">
					...
					<div class="module">
						<h1>
							<span></span><a href="{$goods_cat.url}" class="f6">{$goods_cat.name|escape:html}</a>
						</h1>
						<div class="content">
							<ul class="goods_grid">
					...
					<div class="module">
						<div class="caption">
							<span><a href="../search.php?intro=hot">更多>></a> </span><font
								style="float: left">{$lang.hot_goods}</font>
					...
					<div class="module">
						<div class="caption">
							<span><a href="../search.php?intro=new">更多>></a> </span><font
								style="float: left">{$lang.new_goods}</font>
							<div class="switchover" id="itemNew">
								...
								<h2 class="h2bg">
									<a href="javascript:void(0)"
										onclick="change_tab_style('itemNew', 'h2', this);get_cat_recommend(2)">{$rec_data.cat_name}</a>
								</h2>
								{/foreach} {/if}
							</div>
						</div>
					...
					<div class="module">
						<div class="caption">
							<span><a href="../search.php?intro=promotion">更多>></a> </span>{$lang.promotion_goods}
						</div>
						<div class="content">
							<ul class="goods_grid">
								...
								<li>
									<p class="img">
										<a href="{$goods.url}"><img src="{$goods.thumb}"
												border="0" alt="{$goods.name|escape:html}" /> </a>
									</p>
									<p class="name">
										<a href="{$goods.url}" title="{$goods.name|escape:html}">{$goods.short_name|escape:html}</a>
									</p>
					...
					<div class="module">
						<div class="caption">
							<span> <a href="auction.php">更多>></a> </span>
							{$lang.auction_goods}
						</div>
						<div class="content">
								...
								<li>
									<p class="img">
										<a href="{$auction.url}"><img src="{$auction.thumb}"
												alt="{$auction.goods_name|escape:html}" class="goodsimg" />
										</a>
									</p>
									<p class="name">
										<a href="{$auction.url}"
											title="{$auction.goods_name|escape:html}">{$auction.short_style_name|escape:html}</a>
									</p>
									<p class="price">
			...
			<div id="links">
				<p>
					...
					<a href="{$link.url}" target="_blank" title="{$link.name}"><img
							src="{$link.logo}" alt="{$link.name}" border="0" /> </a>
					<!--结束图片类型的友情链接{/foreach}-->
				</p>
				<p>
					<!-- {if $txt_links} -->
